...
class SARSALB:
    ...
    def initialize_E(self):
        self.E = np.zeros((self.state_num * len(self.actions), 1))
    ...
    def policy_evaluation(self):
        ...
        while True:
            next_pair, R = self.world.iter_step(pacman_direction)
            pacman_direction = np.random.choice(self.world.pacman_action_list, 1, p=self.policy_matrix[
            ...
            self.E[self.transform_pair_index(pair)] += 1
            delta = R + self.gamma * self.Q[self.transform_pair_index(next_pair)] - self.Q[self.transform_pair_index(pair)]
            for tmp_pair_index in range(self.state_num * len(self.actions)):
                self.Q[tmp_pair_index] += self.alpha * delta * self.E[tmp_pair_index]
                self.E[tmp_pair_index] = self.gamma * self.lbda * self.E[tmp_pair_index]
