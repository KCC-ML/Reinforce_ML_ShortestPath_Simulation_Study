        ...
        while True:
            ...
            if tau >= 0:
                ...
                for i in range(tau + 1, min(tau + self.n, T) + 1):
                    G += self.gamma ** (i - tau - 1) * R[i - 1]
                if tau + self.n < T:
                    G += (self.gamma ** self.n) * self.Q[self.transform_pair_index(pairs[tau + self.n])]
                self.Q[self.transform_pair_index(pairs[tau])] += self.alpha * (G - self.Q[self.transform_pair_index(pairs[tau])])
