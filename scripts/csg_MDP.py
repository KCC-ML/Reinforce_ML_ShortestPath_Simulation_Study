...
class MDP:
    def __init__(self, canvas_grid):
        ...
        self.action = [0, 1, 2]
        self.theta = 0.00001
        self.R = np.ones((self.State_sum, 1)) * -1
    ...
    def state_transition_matrix(self, post_P, State_no, improved_policy):
        P = post_P
        ...
        if self.walls[direction, row, col] == 1:
            P[State_no, State_no - direction + ((direction - 1) % 4)] = improved_policy[1]
            P[State_no, State_no - direction + ((direction + 1) % 4)] = improved_policy[2]
            P[State_no, State_no] = improved_policy[0]
            ...
            if direction % 4 == 0:
                ...
                tmp = -4 * 1
            P[State_no, State_no + tmp] = improved_policy[0]
    ...
    # improve 한다
    def policy_improvement(self, next_V, State_no):
        direction = State_no % 4
        row = (State_no // 4) // self.grid_dim
        col = (State_no // 4) % self.grid_dim
        if self.walls[direction, row, col] == 1:
    ...
    def policy_iteration(self):
        ...
        for i in range(self.State_sum):
            next_P = self.state_transition_matrix(self.initial_P, i, self.initial_policy)
        next_V = self.policy_evaluation(self.R, next_P, self.initial_V)
        for i in range(self.State_sum):
            next_P = self.state_transition_matrix(next_P, i, self.policy_improvement(next_V, i))

        while True:
            self.iteration += 1
            print("iteration: ", self.iteration)
            post_V = next_V
            next_V = self.policy_evaluation(self.R, next_P, post_V)
            for i in range(self.State_sum):
                next_P = self.state_transition_matrix(next_P, i, self.policy_improvement(next_V, i))

            if self.theta > abs(min(next_V - post_V)):
                optimal_policy = next_P
                print(next_V)
                break
