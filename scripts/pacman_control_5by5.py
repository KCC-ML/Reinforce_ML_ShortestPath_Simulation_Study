...
class World:
    ...
    def main(self):
        ...
        model = MDP(self.cv, self.pacman.goal_position())
        self.optimal = model.policy_iteration()
    ...
    def iter_step(self):
        while True:
            ...
            pacman_direction = np.random.choice(self.pacman_action_list, 1, p=self.optimal[self.pacman.position[0] * (4 * 5) + self.pacman.position[1] * 4 + pacman_cardinal_point])
