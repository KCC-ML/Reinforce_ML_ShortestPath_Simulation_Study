...
class QLearning:
    ...
    def policy_evaluation(self):
        ...
        while True:
            ...
            next_state_index = self.transform_s_index(next_s)
            self.Q[pair_index] += self.alpha * (R + self.gamma * np.max(self.Q[next_state_index : next_state_index + len(self.actions)]) - self.Q[pair_index])
    ...
    def transform_s_index(self, state):
        unit_row = self.agent_direction_count * self.world.grid_dim * len(self.actions)
        unit_col = self.agent_direction_count * len(self.actions)
        return state[0] * unit_row + state[1] * unit_col + state[2] * len(self.actions) + 0
